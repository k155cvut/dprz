...
**Training Samples Manager** má v sobě již předdefinované klasifikační třídy. Ty ale můžeme smazat a vytvořit si své vlastní. Stejně tak můžeme změnit název celého klasifikačního schématu z defaultního *NLCD2011*. Po vytvoření nového klasifikačního schématu doporučuji si toto schéma uložit, kdybychom se pak chtěli k tvorbě trénovacích ploch vrátit (proto je dobré mít panel co nejvíce roztažený, aby byla vidět všechna funkcionalita). Trénovací plochy zde poté můžeme tvořit buď pomocí jednotlivých bodů či kreslením různých geometrických obrazců. Nicméně asi jako nejjednodušší možnost se nabízí využít jednotlivých segmentů jako trénovacích ploch. Jednotlivé segmenty můžeme vybírat pomocí nástroje **Segment Picker**. Následně si tedy označíme třídu, pro kterou chceme trénovací plochy sbírat, a můžeme začít klikat na jednotlivé segmenty.
